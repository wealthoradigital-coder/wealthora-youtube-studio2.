...
.sl .st Img{font-size:11px;font-weight:600;color:var(--g4);line-height:1.2;text-align:center;}
...
<div class="panel" id="p1">
  ...
  <div class="ssection" id="sec-engine">
    <div class="card">
      <div class="ctitle">Premium automated Video Builder Engine</div>
      <div class="al al-info">This interface compiles scripts, narration audio, dynamic subtitles, templates, and contextual visuals into a finalized premium export.</div>
      
      <label>Video Topic &amp; Parameter Prompt</label>
      <input type="text" id="engine-prompt" placeholder="Video Topic prompt parameters calculated from Phase 1 configuration...">
      
      <div class="g2">
        <div>
          <label>Narration Persona Selector</label>
          <select id="engine-voice">
            <option value="en-US-Christopher">En-US-Christopher - Deep Professional Male (Daniel Core Clone)</option>
            <option value="en-GB-Sonia">En-GB-Sonia - Premium Explainer Female</option>
            <option value="en-US-Arthur">En-US-Arthur - Warm Case-Study Narrative Male</option>
          </select>
        </div>
        <div>
          <label>Subtitle Template Style</label>
          <select id="engine-subs">
            <option value="wealthora-gold">Wealthora Classic (Gold Text, Thick Navy Outline)</option>
            <option value="minimal-modern">Minimalist White (Bold DM Sans, Clean Drop Shadow)</option>
            <option value="beast-impact">High Impact Kinetic (Yellow, Autoshrink Box Accent)</option>
          </select>
        </div>
      </div>
      
      <button class="btn-next" style="background: linear-gradient(135deg, #22c55e, #16a34a); color: white;" id="engine-build-btn" onclick="buildMasterAssetVideo()">Generate Master Asset Video 🚀</button>
      
      <div id="engine-status-area" style="display:none; margin-top:14px;"></div>
      
      <div id="engine-output-player" style="display:none; margin-top:14px;">
        <div class="ctitle">Rendered Production Preview</div>
        <div class="video-preview-box">
          <div class="video-playing-ui">
            <span style="background: rgba(0,0,0,0.6); padding: 4px 8px; font-size: 11px; font-weight: bold; border-radius: 4px; color: var(--gold); align-self: flex-start;" id="preview-tag-badge">1080P | H.264 | Ready</span>
            <div style="font-family:'DM Sans'; font-size: 16px; font-weight: 800; color: #fff; text-align: center; text-shadow: 0 0 4px var(--navyd), 2px 2px 0px var(--navyd); width: 80%; margin: 0 auto;" id="preview-subtitles-simulation">
              "The single biggest operational bottleneck in high-ticket systems is manual tracking..."
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; font-size: 12px; color: #ccc;">
              <span>▶ 0:14 / 8:45</span>
              <span style="color: var(--ok);">● Dynamic Visual Tracking Active</span>
            </div>
          </div>
        </div>
        <div class="btn-row" style="margin-top:10px;">
          <button class="btn btn-gold btn-sm" onclick="toast('Asset synchronized! High-speed download triggered.')">Download Premium Master Video</button>
          <button class="btn btn-out btn-sm" onclick="toast('Video sequence markers pushed directly to Phase 3 Publish Hub!')">Sync to Production Tracker</button>
        </div>
      </div>
    </div>
  </div>
...
<div class="panel" id="p2">
  ...
  <div class="ssection" id="sec-analytics">
    ...
    <div class="card"><div class="ctitle">Sunday Analytics Checklist</div><div class="chklist"><div class="chi" onclick="tc(this)"><input type="checkbox"><label>CTR below 3%? Changed thumbnail text and matched hook</label></div><div class="chi" onclick="tc(this)"><input type="checkbox"><label>Retention drops in first 30s? Tightened script hook intro loop</label></div><div class="chi" onclick="tc(this)"><input type="checkbox"><label>Low sales clickthrough? Validated comment pinning and description visibility</label></div></div></div>
  </div>
  <div class="pnav"><button class="pbk" onclick="goTo(1)">Back to Phase 2</button></div>
</div>
</div>
...
var productNames={
  agency:"Agency Performance Stack",
  realestate:"Real Estate Content Matrix",
  fatigue:"Digital Fatigue Reset",
  intentiq:"IntentIQ",
  bizbox:"BizBox AI Pro"
};

var styleHints={
  educational:"Focuses on solving specific friction points, answering high-intent search phrases.",
  listicle:"High click-through architecture. Creates strong comparative retention over ranked lists.",
  story:"Case study methodology built around single primary transformations.",
  mistake:"Negative cognitive bias trigger. Maximizes programmatic traffic volume.",
  controversial:"Industry myth shattering system to establish high velocity authority.",
  flywheel:"Core foundational asset. Long-form video converting direct intent into ecosystem pipeline sales.",
  shorts:"60-second high loop retention asset directing attention back to central pillars."
};

var formulaHints={
  ppb:"Problem, Promise, Bridge - Fast paced technical frameworks.",
  aida:"Attention, Interest, Desire, Action - Classical product conversion pipeline.",
  pas:"Problem, Agitate, Solve - Best for scaling latent painful realignments.",
  bab:"Before, After, Bridge - Highly structural conversion modeling.",
  storysell:"Open loop narrative - Relatable character perspective framework.",
  listicle:"Countdown tension - Retentive ranking build model.",
  myth:"Instant Authority - Aggressive pattern interrupt pattern.",
  qab:"Question-Answer Bridge - Pure search discovery framework alignment.",
  case:"Highest Trust - Documented performance optimization metrics.",
  todd:"Value Ladder Engine - Structural high value reciprocity layering."
};
...
function goTo(idx){
  for(var i=0;i<3;i++){
    var p=document.getElementById('p'+i);var si=document.getElementById('si'+i);var cn=document.getElementById('cn'+i);
    ...
    if(cn){
      cn.classList.remove('done');
      if(i<idx)cn.classList.add('done');
    }
  }
...
function showSec(id,btn){
  var sections=btn.parentNode.parentNode.getElementsByClassName('ssection');
  for(var i=0;i<sections.length;i++)sections[i].classList.remove('active');
  document.getElementById(id).classList.add('active');
  var tabs=btn.parentNode.getElementsByClassName('stab');
  for(var j=0;j<tabs.length;j++)tabs[j].classList.remove('active');
  btn.classList.add('active');
}

function onStyleChange(){
  var val=document.getElementById('s-style').value;var hint=document.getElementById('style-hint');
  if(styleHints[val]){hint.textContent=styleHints[val];hint.style.display='block';}
  else hint.style.display='none';
}

function onFormulaChange(){
  var val=document.getElementById('s-formula').value;var hint=document.getElementById('formula-hint');
  if(formulaHints[val]){hint.textContent=formulaHints[val];hint.style.display='block';}
  else hint.style.display='none';
}

function tc(el){
  el.classList.toggle('ticked');var chk=el.querySelector('input[type=checkbox]');
  if(chk)chk.checked=el.classList.contains('ticked');
}

function generateIdeas(){
  var p=document.getElementById('s-product').value;
  var st=document.getElementById('s-style').value;
  var prodName=productNames[p]||p;
  
  var baseIdeas=[
    "The Hidden Operational Bottleneck Stopping Your Scaling Potential",
    "How We Automated Core Product Delivery Cycles Under 4 Minutes",
    "Why Most System Architectures Crash Right Before 10k Traction",
    "7 Framework Levers Transforming Broken Operations This Week",
    "The Definitive Automation Matrix Strategy Manual for Creators",
    "Stop Managing Manual Chaos: Build Sustainable Engine Infrastructure",
    "Case Study: Zero to High Ticket Retainer Validation Architecture",
    "The Anti-Fatigue Workspace Layout Engineered for High Focus Production",
    "Automating Lead Qualification Intent Tracking Systems without Ads",
    "The Missing Blueprint Component in Modern Automated Content Models"
  ];

  var listEl=document.getElementById('idea-list');listEl.innerHTML='';
  baseIdeas.forEach(function(idea, i){
    var vScore=100 - (i*4) - Math.floor(Math.random()*3);
    var vClass=vScore>=90?'vhigh':(vScore>=80?'vmed':'vlow');
    var item=document.createElement('div');
    item.className='idea-item'+(i===0?' sel':'');
    item.onclick=function(){
      var items=listEl.getElementsByClassName('idea-item');
      for(var j=0;j<items.length;j++)items[j].classList.remove('sel');
      item.classList.add('sel');
      selectIdea(idea, prodName);
    };
    item.innerHTML='<div class="ichk">✓</div><div style="flex:1;"><div class="itxt">'+idea+'</div><div class="imeta">Viral Score: <span class="'+vClass+'">'+vScore+'%</span> | Target Match: High Intent</div></div>';
    listEl.appendChild(item);
  });

  document.getElementById('idea-card').style.display='block';
  selectIdea(baseIdeas[0], prodName);
  toast("Generated 10 custom product-aligned concepts!");
}

function selectIdea(title, prodName){
  document.getElementById('custom-title').value=title;
  document.getElementById('key-points').value=
    "- Core Problem: High operating friction & unstructured asset deployment patterns.\n" +
    "- Core Insight: Moving from raw chaotic labor into systematic software-lite optimization loops.\n" +
    "- Specific Vehicle: Leveraging the unique technical mechanisms inside " + prodName + ".\n" +
    "- Hard CTA Call: Access the system waitlist via the pinned link down below.";
}

function proceedToPhase2(){
  var title=document.getElementById('custom-title').value;
  if(!title){toast("Please select or specify a structural video title first."); return;}
  ...
  S.kp=document.getElementById('key-points').value;

  var pName=productNames[S.pk]||S.pk;
  
  // Render Data Carrying Pills
  var pillsHtml='<div class="cp">Title: <span>'+S.title+'</span></div>'+
                '<div class="cp">Product: <span>'+pName+'</span></div>'+
                '<div class="cp">Formula: <span>'+S.formula.toUpperCase()+'</span></div>'+
                '<div class="cp">Tone: <span>'+S.tone+'</span></div>';
  document.getElementById('ctx-pills').innerHTML=pillsHtml;
  document.getElementById('ctx-pills-3').innerHTML=pillsHtml;

  // Auto-populate Phase 2 Asset Engine parameter prompt
  document.getElementById('engine-prompt').value = "PRODUCE VIDEO: '" + S.title + "' | ALIGNMENT: " + pName + " (" + S.formula.toUpperCase() + " Formula) | VOICE TONE: " + S.tone;

  // Build Assets Simulation
  document.getElementById('script-out').classList.remove('empty');
  document.getElementById('script-out').textContent=
    "[HOOK PHASE - " + S.formula.toUpperCase() + " START]\n" +
    "Look closely at the high-ticket systems crashing around you. It isn't a lack of drive or work ethic. It is an architecture problem. Most operations are running on pure manual chaos—burning human capital on jobs that simple code engines can handle in milliseconds.\n\n" +
    "[AGITATION / ELABORATION POINTERS]\n" +
    "Every single time you manually copy a dataset, track intent by hand, or draft copy without automated leverage models, you leak premium margin. Here is the transformation sequence documented explicitly through our structural milestones:\n" + S.kp + "\n\n" +
    "[SYSTEM SOLVE & DEMONSTRATION]\n" +
    "This is exactly why we spent weeks perfecting the mechanical loops inside " + pName + ". It cuts out production exhaustion entirely, enabling small systems to hit maximum viral distribution tracking loops seamlessly.\n\n" +
    "[ACQUISITION CONVERSION OUTRO]\n" +
    "Do not keep building broken bridges with old manual rules. Go below, look at the pinned link, and lock your operational access right now. See you inside.";

  document.getElementById('wc-display').textContent="(" + Math.floor(450 + Math.random()*200) + " words - Optimized Asset)";

  // Alternatives
  document.getElementById('hook-card').style.display='block';
  document.getElementById('hook-out').textContent="1. Stop trading raw manual hours for highly unstable operational output. Here is the framework strategy.\n2. This single architectural mistake destroys roughly 82% of young creator retention ecosystems early on.";
  
  document.getElementById('cta-card').style.display='block';
  document.getElementById('cta-out').textContent="1. Stop missing high intent indicators. Click the pinned asset below and deploy this engine now.\n2. Access the complete operational manual at the top of the description instantly.";

  // SEO Package
  document.getElementById('seo-out').classList.remove('empty');
  document.getElementById('seo-out').textContent=
    "⭐⭐ HIGH INTENT TITLES ⭐⭐\n" +
    "1. " + S.title + " (Systematic Scaling Blueprint)\n" +
    "2. How to Automate " + pName + " Without Coding Skills\n\n" +
    "TARGET SEARCH TAGS:\n" +
    S.pk + ", operations management, system automation, workflow builder, productivity engine, digital growth, wealthora automation\n\n" +
    "TIMESTAMPS / CHAPTERS:\n" +
    "0:00 The Manual Execution Trap\n" +
    "1:45 Breaking Down Core System Friction\n" +
    "4:12 Deploying Automated Software-Lite Accelerators\n" +
    "7:30 Real-Time Transformation Blueprint\n" +
    "8:15 Actionable Conversions & Outro Architecture";

  // Thumbnail Brief
  document.getElementById('thumb-out').classList.remove('empty');
  document.getElementById('thumb-out').textContent=
    "CANVA PRODUCTION SPECIFICATION SHEET:\n" +
    "- BACKGROUND: Deep luxury corporate navy overlay gradient with dark geometric technical grid line design.\n" +
    "- SUBJECT: Clean, crisp floating mockups of the " + pName + " interface illuminated with subtle neon glow.\n" +
    "- TEXT ELEMENT: Bold, high-contrast typography in Pure Gold (#E8C800) and White. 3-4 words absolute max.\n" +
    "- CORE CONCEPT TRIGGER: Visual juxtaposition showing a red 'MANUAL CHAOS' tag arrow crossing out into a glowing green 'AUTOMATED ENGINE' badge.";

  // Description
  document.getElementById('desc-out').classList.remove('empty');
  document.getElementById('desc-out').textContent=
    "Stop wasting hours maintaining technical friction loops manually. In this breakdown video, we expose the underlying system architecture frameworks that change the game completely.\n\n" +
    "🔥 Deploy the complete technical solution engine instantly here: [YOUR_SELAR_OR_WAITLIST_URL]\n\n" +
    "Resources Referenced In This Production:\n" +
    "👉 Get premium voice assets from ElevenLabs: https://elevenlabs.io/?from=affiliate\n" +
    "👉 Access design canvas layouts from Canva Pro: https://canva.com\n\n" +
    "Make sure to watch our comprehensive foundational masterclass next to tie your pipeline end-to-end.";

  document.getElementById('fwd-banner').classList.add('show');
  
  // Track entry initialization
  if(!pipeline.some(function(item){return item.title===S.title;})){
    pipeline.push({title:S.title, product:pName, status:'In Progress', date:new Date().toLocaleDateString()});
  }
  updateTrackerUI();

  goTo(1);
  toast("Phase 2 content populated perfectly!");
}

function buildMasterAssetVideo() {
  var statusArea = document.getElementById('engine-status-area');
  var player = document.getElementById('engine-output-player');
  var buildBtn = document.getElementById('engine-build-btn');
  ...
  try {
    var steps = [
      { msg: "🔄 Synchronizing target script assets...", time: 800 },
      { msg: "🎙️ Generating premium voiceovers via ElevenLabs digital clone matrix...", time: 1200 },
      { msg: "🎨 Rendering dynamic kinetic subtitle markers & navy alignment lines...", time: 1000 },
      { msg: "🎞️ Matching contextual B-Roll parameters and rendering background audio loop (12%)...", time: 1400 },
      { msg: "🚀 Finalizing multi-track video stream compilation...", time: 900 }
    ];

    function runStep(i) {
      if (i < steps.length) {
        var p = document.createElement('div');
        p.className = "al al-info";
        p.innerHTML = '<span class="spin"></span> ' + steps[i].msg;
        statusArea.appendChild(p);
        window.scrollTo({top: statusArea.offsetTop, behavior: 'smooth'});
        
        setTimeout(function() {
          p.className = "al al-ok";
          p.innerHTML = "✅ " + steps[i].msg.replace("...", " completed.");
          runStep(i + 1);
        }, steps[i].time);
      } else {
        ...
        setTimeout(function() {
          ...
          // Seed active simulated subtitles text based on the product selected
          var pName = productNames[S.pk] || "Wealthora Systems";
          document.getElementById('preview-subtitles-simulation').textContent = 
            "\"Deploying the automated loops within " + pName + " eliminates manual operations entirely...\"";
            
          toast("Premium automated master asset compiled successfully!");
        }, 500);
      }
    }
    ...
    statusArea.innerHTML = '<div class="al al-warn">❌ System Exception Intercepted during master video compilation process: ' + error.message + '</div>';
    buildBtn.disabled = false;
  }
}

function regenScript(){
  toast("Re-optimizing behavioral text hooks matching selected tone rules...");
  proceedToPhase2();
}
function regenHook(){ toast("Generating fresh high click-through patterns..."); }
function regenCTA(){ toast("Compiling high authority conversion metrics..."); }

function downloadScript(){
  var txt = document.getElementById('script-out').textContent;
  var blob = new Blob([txt], {type: "text/plain;charset=utf-8"});
  var a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = S.title.toLowerCase().replace(/[^a-z0-9]+/g, "-") + "-script.txt";
  a.click();
}
...
function updateTrackerUI(){
  ...
  pipeline.forEach(function(item, idx){
    var tr = document.createElement('tr');
    var badgeColor = item.status==='Published'?'#4ade80':'var(--gold)';
    tr.innerHTML = '<td><strong>'+item.title+'</strong></td>'+
                   '<td>'+item.product+'</td>'+
                   '<td><span style="color:'+badgeColor+'; font-weight:bold;">'+item.status+'</span></td>'+
                   '<td>'+item.date+'</td>'+
                   '<td><button class="btn btn-out btn-sm" style="padding:3px 8px; font-size:10px;" onclick="toggleStatus('+idx+')">Toggle Status</button></td>';
    tbody.appendChild(tr);
  });
  
  document.getElementById('vid-count').textContent = pipeline.length + " video(s) tracked";
  
  var pub = pipeline.filter(function(v){return v.status==='Published';}).length;
  var mbEl2 = document.getElementById('m-bar');
  var mcEl2 = document.getElementById('m-count');
...
function toggleStatus(idx){
  if(pipeline[idx]){
    pipeline[idx].status = pipeline[idx].status === 'In Progress' ? 'Published' : 'In Progress';
    updateTrackerUI();
    toast("Pipeline progress status synced!");
  }
}

function markPublished(){
  var currentItem = pipeline.find(function(v){return v.title === S.title;});
  if(currentItem) {
    currentItem.status = 'Published';
  } else if (S.title) {
    var pName = productNames[S.pk] || S.pk;
    pipeline.push({title:S.title, product:pName, status:'Published', date:new Date().toLocaleDateString()});
  }
  updateTrackerUI();
  toast("Marked as Published! Metric calculations recalculated in tracking ledger.");
  goTo(2);
  showSec('sec-tracker', document.querySelector('[onclick*="sec-tracker"]'));
}

function newVideo(){
  S={pk:'agency',title:'',formula:'ppb',tone:'authoritative',length:'medium',style:'educational',kp:''};
  document.getElementById('idea-card').style.display='none';
  document.getElementById('custom-title').value='';
  document.getElementById('key-points').value='';
  document.getElementById('idea-list').innerHTML='';
  document.getElementById('fwd-banner').classList.remove('show');
  document.getElementById('hook-card').style.display='none';
  document.getElementById('cta-card').style.display='none';
  document.getElementById('wc-display').textContent='';
  document.getElementById('style-hint').style.display='none';
  document.getElementById('formula-hint').style.display='none';
  document.getElementById('engine-output-player').style.display='none';
  goTo(0);
  toast("Application state cleared. Input new configurations.");
}
</script>
</body>
</html>
